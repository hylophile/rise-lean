...
fn t_scaliiintel_fail() {
    let goals = "(term_mvar n_3)=(* (* 4 128) 128);(array (term_mvar n_15) (type_mvar s_16))=(array (term_mvar m_26) (array 128 (type_mvar t_27)));(-> (type_mvar s_16) (type_mvar t_17))=(-> (array (term_mvar n_18) (type_mvar s_19)) (array (term_mvar n_18) (type_mvar t_20)));";
    let r = unify(goals).unwrap();
    assert_eq!(r, map![]);
}
...
fn t_wtf3() {
    ...
    let goals = "(term_mvar n_1)  = (+ (- (+ (term_bvar h_1) 6) 1) -2)";
    let r = unify(goals).unwrap();
    assert_eq!(r, map![]);
}
...
#[test]
fn unsound() {
    let goals = "(term_mvar n_0)=(+ (+ 5 5) -10);(term_mvar m_1)=0;(term_mvar p_2)=(/ (term_bvar h_5) (term_mvar m_1))";
    let goals = "(term_mvar n_0)=(* (term_mvar m_2) (term_mvar p_3));(term_mvar n_0)=2;(term_mvar)";
    let r = unify(goals).unwrap();
    pp(&r);
    assert_eq!(r, map![]);
}
