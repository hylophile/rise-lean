...
private def getFreshAccessIdentifier (name : String := "tmp") : InferM String := do
...
def typ (dt : RData) : CType :=
    match dt with
        ...
        | .natType => .scalar (.int)
        | .index _ => .scalar (.int)
        | .array n dt => .array (typ dt) (some (getNat n))
...
partial def generateAccess (dt : RData) (expr : CExpr) (path: Path) (env : Environment) (cont : CExpr → CStmt) : CStmt :=
    match path with
        | [] => cont expr
        ...
        | PathExpr.cIntExpr _ :: _ => match dt with
                                        | .array _ _ => let (dt2, k, ps) := flattenArrayIndicies dt path
                                                        generateAccess dt2 (.arraySubscript expr (.arithmeticExpr k)) ps env cont
...
partial def cmd (env : Environment) (p : DPIAPhrase) : CStmt :=
    match p with
        | ⟨.ifThenElse cond thenP elseP, _ ⟩ => cond |> exp env [] (fun c => .ifThenElse c (cmd env thenP) (cmd env elseP))
        | ⟨.bvar i n, .comm ⟩ => lookUpCommEnv env n i
        | ⟨.app ⟨.bvar _ _, _⟩ e, _ ⟩ => .breakStmt -- needs fixing here comes contEnv into play
...
partial def cmdImp (env : Environment) (imp : ImperativePrimitives) : CStmt :=
    match imp with
        | .skip => .comment "skip"
        | .seq p1 p2=> .stmts (p1 |> cmd env) (p2 |> cmd env)
        | .assign _ a e => e |> exp env [] (fun x =>
                            a |> acc env [] (fun y =>
                                .exprStmt (.assignment x y)))
...
--        | .newDoubleBuffer n _ _ dt inP outP ⟨.lam ps t p, _⟩ => codeGenNewDoubleBuffer (.array n dt) inP outP (mkBvar 0 ps t) p env  --- take a look at names again, not important for dot product
        | .forLoop u n b => match b.node with
                                | .lam i t p => codeGenFor n (mkBvar 0 i t) p u env
                                | _ => panic! s!"the for-body should be a function"
        | .forNat u n b => match b.node with
                                | .deplam i (.rise .nat) p => codeGenForNat n (.bvar 0 i) p u env
                                | _ => panic! s!"the for-body should be a function"
...
private partial def acc (env : Environment) (path : Path) (cont : CExpr → CStmt) (p : DPIAPhrase) : CStmt :=
    match p with
        | ⟨.bvar idx name, .acc dt⟩ =>  let expr := lookUpIdentEnv env name idx
                                        generateAccess dt expr path env cont
        | ⟨.imperative imp, _⟩ => accImp env path cont imp
        | ⟨.proj1 p, _⟩ => (liftPair p).1 |> acc env path cont
        | ⟨.proj2 p, _⟩ => (liftPair p).1 |> acc env path cont
        | _ => panic! s!"don't know how to generate code for {p.node}"

private partial def accImp (env : Environment) (path : Path) (cont : CExpr → CStmt) (imp : ImperativePrimitives): CStmt :=
    match imp with
        ...
        | .scatterAcc n m _ y a =>  match path with
                                        | (.cIntExpr i) :: ps => let id := RNat.bvar 0 (mkName "i")
                                                                 mkIdx n (.index m) (mkNatAsIndex n {node := .natural i, type := .expr .natType .read}) y |>
                                                                    exp env [] (fun yic =>
                                                                                            .block [.declStmt (.var (mkName "i") (.scalar .int) (some yic)),
...
private partial def exp (env : Environment) (path : Path) (cont : CExpr → CStmt) (p : DPIAPhrase) : CStmt :=
    match p with
        ...
        | ⟨.lit v, dt⟩ => match path with
                            | [] => match getDataType dt with
                                        ...
                                        | _ => panic! s!"expected an index type or scalar type"
                            | (.cIntExpr _) :: _ => panic! s!"this implementation has no array data yet!"
                            | _ => panic! s!"unexpected path {path}"
        | ⟨.natural n, _⟩ => match path with
                                | [] => .exprStmt (.arithmeticExpr  n)
                                | _ => panic! s!"exprected the path to be empty"
        | ⟨.functional func, dt⟩ => expFunc env path cont func (getDataType dt)
...
private partial def expFunc (env : Environment) (path : Path) (cont : CExpr → CStmt) (func : FunctionalPrimitives) (dt : RData): CStmt :=
    match func with
        | .neg v => match (dt, path) with
                        | (.scalar _, []) => exp env [] (fun e => cont (codeGenUnaryOp .minus e)) v
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .not v => match (dt,path) with
                        | (.scalar _, []) => exp env [] (fun e => cont (codeGenUnaryOp .ex e)) v
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .add n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.plus) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .sub n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.minus) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .div n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.div) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .mul n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.mult) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .mod n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.mod) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .gt n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.greaterThan) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .lt n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.lessThan) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .equal n m => match (dt, path) with
                        | (.scalar _, []) | (.natType, []) => exp env [] (fun e1 => exp env [] (fun e2 => cont (codeGenBinaryOp (.equal) e1 e2)) m) n
                        | _ => panic! s!"the path has to be empty and the data type has to be a scalar but got {path} and {dt} "
        | .cast _ dt e => match path with
                            | [] => exp env [] (fun x => cont (.cast (typ dt) x)) e
                            | _ => panic! s!"expected the path to be empty for cast"
        | .indexAsNat _ e => exp env path cont e
        | .natAsIndex _ e => exp env path cont e
        | .split n _ _ _ e => match path with
                                | (.cIntExpr i) :: (.cIntExpr j) :: ps => exp env ((.cIntExpr (.mult n (.mult i j)))::ps) cont e
                                | _ => panic! s!"expected two C-Integer-Expressions on the path"
        | .join _ m _ _ e => match path with
                                | (.cIntExpr i) :: ps => exp env ((.cIntExpr (.div m i)) :: (.cIntExpr (.div m i)) :: ps) cont e
        ...
        | .zip n dt1 dt2 _ e1 e2 => match path with
                                        | (.cIntExpr i) :: (.pairAccess xj) :: ps => match xj with
                                                                                        | .fstMember => exp env ((.cIntExpr i) :: ps) cont e1
                                                                                        | .sndMember => exp env ((.cIntExpr i) :: ps) cont e2
        ...
        | .unzip _ _ _ _ e => match path with
                                | (.pairAccess xj) :: (.cIntExpr i) :: ps => exp env ((.cIntExpr i) :: (.pairAccess xj) :: ps) cont e
                                | _ => panic! s!"expected a tupel access followed by a C-Integer-Expression on the path but got {path}"
        --| .depZip _ _ _ e1 e2 =>                      --- did not implement dependen prims
        | .makePair _ _ _ e1 e2 => match path with
                                    | (.pairAccess xj) :: ps => match xj with
                                                                    | .fstMember => exp env ps cont e1
                                                                    | .sndMember => exp env ps cont e2
                                    | [] => e1 |> exp env [] (fun ec1 =>
        ...
        --                                                         e |> exp env ((.cIntExpr ((getNat n) -1)) :: ps) (fun right =>
        --                                                             genPad))
...
partial def codeGenNew (dt : RData) (v p : DPIAPhrase) (env : Environment) : CStmt :=
    match v with
...
partial def codeGenFor (n : RNat) (i p : DPIAPhrase) (unroll : Bool) (env : Environment) : CStmt :=
    .breakStmt
    --  let _ := assertUnroll unroll
    --  let cI :=

partial def codeGenForNat (n : RNat) (i : RNat) (p : DPIAPhrase) (unroll : Bool) (env : Environment) : CStmt :=
    .breakStmt

partial def codeGenIdx (i e : DPIAPhrase) (env : Environment) (ps : Path) (cont : CExpr → CStmt) : CStmt :=
    i |> exp env [] (fun x => match x with
                                | .declRef n => e |> exp env ((.cIntExpr (.bvar 0 n)) :: ps) cont
                                | .arithmeticExpr ae => e |> exp env ((.cIntExpr ae) :: ps) cont
                                | _ =>  let arithVar := RNat.bvar 0 (mkName "idx")
                                        CStmt.block [.declStmt (.var (mkName "idx") (.scalar .int) (some x)),
                                                     e |> exp env ((.cIntExpr arithVar) :: ps) cont])

partial def codeGenIdxAcc (i a : DPIAPhrase) (env : Environment) (ps : Path) (cont : CExpr → CStmt) : CStmt :=
    i |> exp env [] (fun x => match x with
                                | .literal text => a |> acc env ((.cIntExpr (toNat text)) :: ps) cont
                                | .declRef name => a |> acc env ((.cIntExpr (.bvar 0 name)) :: ps) cont
                                | .arithmeticExpr ae => a |> acc env ((.cIntExpr ae) :: ps) cont
                                | _ => let arithVar := RNat.bvar 0 (mkName "idxAcc")
                                       .block [.declStmt (.var (mkName "idxAcc") (.scalar .int) (some x)),
