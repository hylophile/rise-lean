                            ...
                            mkLam (.fn condType (.fn tExprType fExprFunType)) (mkName "cond") condType
                                   (mkLam (.fn tExprType fExprFunType) (mkName "tExpr") tExprType
                                          (mkLam fExprFunType (mkName "fExpr") fExprType
                                                 {node := (.ifThenElse (mkBvar 2 (mkName "cond") condType) (mkBvar 1 (mkName "tExpr") tExprType) (mkBvar 0 (mkName "fExpr") fExprType)), type := .expr t .read : DPIAPhrase}))
                          ...
                          mkDeplam depType name (.rise .nat)
                                   (mkLam eFunType (Lean.Name.mkSimple "e") eType
                                          (mkNatAsIndex (.bvar 1 name)
                                                        {node:= (.bvar 0 (mkName "e")), type := eType : DPIAPhrase}))
                                                   ...
                                                   (mkLam loadFunType (mkName "load") loadType
                                                        (mkLam eFunType (mkName "e") eType
                                                               (mkCircularBuffer n (.bvar 3 alloc) (.bvar 2 sz) s t
                                                                                    {node := (.bvar 1 (mkName "load")), type := loadType}
                                                                                    {node := (.bvar 0 (mkName "e")), type := eType}))))
                                        ...
                                        (mkLam wrFunType (mkName "wr") wrType
                                               (mkLam eFunType (mkName "e") eType
                                                      (mkRotatevalues n (.bvar 2 sz) s
                                                                      {node := (.bvar 1 (mkName "wr")), type := wrType : DPIAPhrase}
                                                                      {node := (.bvar 0 (mkName "e")), type := eType : DPIAPhrase} )))
                     ...
                     | PhraseType.fn (.expr (.array m (.index n)) .read)
                            ...
                            mkLam yFunType (mkName "y") yType
                                  (mkLam xFunType (mkName "x") xType
                                         (mkGather n m t
                                                   {node := (.bvar 1 (mkName "y")), type := yType : DPIAPhrase}
                                                   {node := (.bvar 0 (mkName "x")), type := xType : DPIAPhrase}))
