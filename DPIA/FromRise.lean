...
def primitives (name : Lean.Name) (type : PhraseType) : DPIAPhrase :=
  match name.toString with
    ...
    | "add" => match type with
                 | .fn (.expr t .read) (.fn (.expr _ .read) (.expr _ .read)) =>
                     let eType := PhraseType.expr t .read
                     mkLam (.fn eType (.fn eType eType)) (mkName "e1") eType
                            (mkLam (.fn eType eType) (mkName "e2") eType
                                   (mkAdd t (mkBvar 1 (mkName "e1") eType)
                                            (mkBvar 0 (mkName "e2") eType)))
    ...
    | "sub" => match type with
                 | .fn (.expr t .read) (.fn (.expr _ .read) (.expr _ .read)) =>
                     ...
                     mkLam (.fn eType (.fn eType eType)) (mkName "e1") eType
                            (mkLam (.fn eType eType) (mkName "e2") eType
                                   (mkSub t (mkBvar 1 (mkName "e1") eType)
                                            (mkBvar 0 (mkName "e2") eType)))
    ...
    | "mul" => match type with
                 | .fn (.expr t .read) (.fn (.expr _ .read) (.expr _ .read)) =>
                     ...
                     mkLam (.fn eType (.fn eType eType)) (mkName "e1") eType
                            (mkLam (.fn eType eType) (mkName "e2") eType
                                   (mkMul t (mkBvar 1 (mkName "e1") eType)
                                            (mkBvar 0 (mkName "e2") eType)))
    ...
    | "div" => match type with
                 | .fn (.expr t .read) (.fn (.expr _ .read) (.expr _ .read)) =>
                     ...
                     mkLam (.fn eType (.fn eType eType)) (mkName "e1") eType
                            (mkLam (.fn eType eType) (mkName "e2") eType
                                   (mkDiv t (mkBvar 1 (mkName "e1") eType)
                                            (mkBvar 0 (mkName "e2") eType)))
    ...
    | "select" => match type with
                     | .fn  (.expr (.scalar .bool) .read)
                            ...
                            mkLam (.fn condType (.fn tExprType fExprFunType)) (mkName "cond") condType
                                   (mkLam (.fn tExprType fExprFunType) (mkName "tExpr") tExprType
                                          (mkLam fExprFunType (mkName "fExpr") fExprType
                                                 (mkIfThenElse (.expr t .read) (mkBvar 2 (mkName "cond") condType)
                                                                               (mkBvar 1 (mkName "tExpr") tExprType)
                                                                               (mkBvar 0 (mkName "fExpr") fExprType))))
    ...
    | "not" => match type with
                  | PhraseType.fn (.expr (.scalar .bool) .read) (.expr (.scalar .bool) .read) =>
                            ...
                            mkLam (.fn eType (.expr (.scalar .bool) .read)) (mkName "e") eType
                                   (mkNot (mkBvar 0 (mkName "e ") eType))
                  | _ => panic! s!"{type} is no valid not type"

    | "gt"  => match type with
                 | .fn (.expr t .read) (.fn (.expr _ .read) (.expr (.scalar .bool) .read)) =>
                     let eType := PhraseType.expr t .read
                     mkLam (.fn eType (.fn eType eType)) (mkName "e1") eType
                            (mkLam (.fn eType eType) (mkName "e2") eType
                                   (mkGt  (mkBvar 1 (mkName "e1") eType)
                                          (mkBvar 0 (mkName "e2") eType)))
    ...
    | "cast" => match type with
                  | .fn  (.expr s .read) (.expr t .read) =>
                     let xType := PhraseType.expr s .read
                     mkLam  (.fn xType (.expr t .read)) (mkName "x") xType
                            (mkCast s t (mkBvar 0 (mkName "x") xType))
                  | _ => panic! s!"{type} is no valid cast type"

    | "indexAsNat" => match type with
                        | .fn (.expr (.index n) .read) (.expr .natType .read) =>
                            let eType := PhraseType.expr (.index n) .read
                            mkLam   (.fn eType (.expr (.index n) .read)) (mkName "e") eType
                                    (mkIndexAsNat n (mkBvar 0 (mkName "e") eType))
                        | _ => panic! s!"{type} is no valid indexAsNat type"

    | "natAsIndex" => match type with
                      | .pi (.rise .nat) name (.fn (.expr .natType .read) (.expr (.index n) .read)) =>
                            ...
                            mkDeplam depType name (.rise .nat)
                                          (mkLam eFunType (mkName "e") eType
                                                 (mkNatAsIndex (.bvar 1 name) (mkBvar 0 (mkName "e") eType)))
                      | _ => panic! s!"{type} is no valid natAsIndex type"

    | "rlet" => match type with
                  | .fn  (.expr s .read)
                         (.fn   (.fn (.expr _ .read) (.expr t a)) (.expr _ _)) =>
                            let fType := PhraseType.fn (.expr s .read) (.expr t .read)
                            let xType := PhraseType.expr s .read
                            let fFunType := PhraseType.fn fType (.expr t a)
                            mkLam  (.fn xType fFunType) (mkName "x") xType
                                   (mkLam fFunType (mkName "f") fType
                                          (mkRlet s t a (mkBvar 1 (mkName "x") xType) (mkBvar 0 (mkName "f") fType)))
                  | _ => panic! s!"{type} is no valid let type"

    | "toMem" => match type with
                    | .fn (.expr t .write) (.expr _ .read) =>
                        let eType := PhraseType.expr t .write
                        mkLam   (.fn eType (.expr t .read)) (mkName "e") eType
                                (mkToMem t (mkBvar 0 (mkName "e") eType))
                    | _ => panic! s!"not implemented yet"

    | "generate" => match type with
                     | .fn  (.fn (.expr (.index n) .read) (.expr t .read))
                            (.expr (.array _ _) .read) =>
                            let fType := PhraseType.fn (.expr (.index n) .read) (.expr t .read)
                            mkLam  (.fn fType (.expr (.array n t) .read)) (mkName "f") fType
                                   (mkGenerate n t (mkBvar 0 (mkName "f") fType))
                     | _ => panic! s!"{type} is no valid generate type"

    | "idx" => match type with
                 | .fn (.expr (.index n) .read)
                       (.fn (.expr (.array _ t) .read) (.expr _ .read)) =>
                     ...
                     mkLam (.fn iType eFunType) (mkName "i") iType
                            (mkLam eFunType (mkName "e") eType
                                   (mkIdx n t
                                          (mkBvar 1 (mkName "i") iType)
                                          (mkBvar 0 (mkName "e") eType)))
                 | _ => panic! s!"{type} is no valid index type"

    | "take" => match type with
                  | .pi (.rise .nat) n (.fn (.expr (.array nm t) .read) (.expr (.array _ _) .read)) =>
    ...
    | "drop" => match type with
                  | .pi (.rise .nat) n (.fn (.expr (.array nm t) .read) (.expr (.array _ _) .read)) =>
                       ...
                       mkDeplam (.pi (.rise .nat) n eFunType) n (.rise .nat)
                                (mkLam eFunType (mkName "e") eType
                                       (mkDrop (.bvar 1 n) (.minus nm (.bvar 0 n)) t (mkBvar 1 (mkName "e") eType)))
                  | _ => panic! s!"{type} is no valid drop type"

    | "concat" => panic! s!"not implemented yet"

    | "split" => match type with
                  | .pi (.rise .nat) n (.fn (.expr (.array _ t) a) (.expr (.array m (.array _ _)) _)) =>
                            let eType := PhraseType.expr (.array (.mult m (.bvar 0 n)) t) a
                            ...
                            mkDeplam depFunType n (.rise .nat)
                                   (mkLam eFunType (mkName "e") eType
                                          (mkSplit (.bvar 1 n) m t a (mkBvar 0 (mkName "e") eType)))
                  | _ => panic! s!"{type} is no valid join type"

    | "join"  => match type with
                  | .fn (.expr (.array n (.array m t)) a) (.expr (.array _ _) _) =>
                            let eType := PhraseType.expr (.array n (.array m t)) a
                            let eFunType := PhraseType.fn eType (.expr (.array (.mult n m) t) a)
                            mkLam eFunType (mkName "e") eType
                                   (mkJoin n m t a (mkBvar 0 (mkName "e") eType))
                  | _ => panic! s!"{type} is no valid join type"


    | "slide" => match type with
                  | .pi (.rise .nat) sz (.pi (.rise .nat) sp (.fn (.expr (.array insz t) .read) (.expr (.array np1 (.array _ _)) .read))) =>
                            ...
                            mkDeplam innerDepType sz (.rise .nat)
                                   (mkDeplam innerDepType sp (.rise .nat)
                                          (mkLam eFunType (mkName "e") eType
                                                 (mkSlide (.minus np1 (.nat 1)) (.bvar 2 sz) (.bvar 1 sp) t (mkBvar 0 (mkName "e") eType))))
    ...
    | "circularBuffer" => match type with
                            | .pi (.rise .nat) alloc
                                ...
                                mkDeplam (.pi (.rise .nat) alloc innerDepType) alloc (.rise .nat)
                                         (mkDeplam innerDepType sz (.rise .nat)
                                                 (mkLam loadFunType (mkName "load") loadType
                                                        (mkLam eFunType (mkName "e") eType
                                                               (mkCircularBuffer n (.bvar 3 alloc) (.bvar 2 sz) s t (mkBvar 1 (mkName "load") loadType) (mkBvar 0 (mkName "e") eType)))))
                            | _ => panic! s!"{type} is no valid circularBuffer type"

    | "rotateValues" => match type with
                            | .pi (.rise .nat) sz (.fn  (.fn (.expr s .read) (.expr _ .write))
                                                        (.fn (.expr (.array insz _) .read) (.expr (.array n _) .read))) =>
                               let eType := PhraseType.expr (.array insz s) .read
                               ...
                               mkDeplam (.pi (.rise .nat) sz wrFunType) sz (.rise .nat)
                                        (mkLam wrFunType (mkName "wr") wrType
                                               (mkLam eFunType (mkName "e") eType
                                                      (mkRotatevalues n (.bvar 2 sz) s (mkBvar 1 (mkName "wr") wrType) (mkBvar 0 (mkName "e") eType))))
                            | _ => panic! s!"{type} is no valid type for rotateValues"

    | "transpose" => match type with
                        | .fn (.expr (.array n (.array m t)) a) (.expr (.array _ (.array _ _)) _) =>
                            let eType := PhraseType.expr (.array n (.array m t)) a
                            let eFunType := PhraseType.fn eType (.expr (.array m (.array n t)) a)
                            mkLam eFunType (mkName "e") eType
                                  (mkTranspose n m t a (mkBvar 0 (mkName "e") eType))
                        | _ => panic! s!"{type} is no valid type for transpose"

    | "gather" => match type with
                     | .fn (.expr (.array m (.index n)) .read)
                           (.fn (.expr (.array _ t) .read) (.expr (.array _ _) .read)) =>
                            ...
                            mkLam yFunType (mkName "y") yType
                                  (mkLam xFunType (mkName "x") xType
                                         (mkGather n m t (mkBvar 1 (mkName "y") yType) (mkBvar 0 (mkName "x") xType)))
                     | _ => panic! s!"{type} is no valid type for gather"

    | "scatter" => match type with
                     | .fn (.expr (.array n (.index m)) .read)
                           (.fn (.expr (.array _ t) .write) (.expr (.array _ _) .write)) =>
                            ...
                            mkLam yFunType (mkName "y") yType
                                  (mkLam xFunType (mkName "x") xType
                                         (mkScatter n m t (mkBvar 1 (mkName "y") yType) (mkBvar 0 (mkName "x") xType)))
                     | _ => panic! s!"{type} is no valid type for scatter"

    | "padCst" => match type with
                     | .pi  (.rise .nat) l
                            (.pi (.rise .nat) q (.fn (.expr t .read)
                                                     (.fn (.expr (.array n _) .read) (.expr (.array _ _) .read)))) =>
                         let eType := PhraseType.expr (.array n t) .read
                         ...
                         mkDeplam (.pi (.rise .nat) l innerDepType) l (.rise .nat)
                                  (mkDeplam innerDepType q (.rise .nat)
                                            (mkLam cstFunType (mkName "cst") cstType
                                                   (mkLam eFunType (mkName "e") eType
                                                          (mkPadCst n (.bvar 3 l) (.bvar 2 q) t (mkBvar 1 (mkName "cst") cstType) (mkBvar 0 (mkName "e") eType)))))
    ...
    | "padClamp" => match type with
                       | .pi (.rise .nat) l
                            ...
                            mkDeplam (.pi (.rise .nat) l innerDepType) l (.rise .nat)
                                     (mkDeplam innerDepType q (.rise .nat)
                                               (mkLam eFunType (mkName "e") eType
                                                      (mkPadClamp n (.bvar 2 l) (.bvar 1 q) t (mkBvar 0 (mkName "e") eType))))
                       | _ => panic! s!"{type} is no valid type for padClamp"

    | "padEmpty" => match type with
                       | .pi (.rise .nat) r (.fn (.expr (.array n t) .write) _) =>
                            ...
                            mkDeplam (.pi (.rise .nat) r eFunType) r (.rise .nat)
                                     (mkLam eFunType (mkName "e") eType
                                            (mkPadEmpty n (.bvar 1 r) t (mkBvar 0 (mkName "e") eType)))
                       | _ => panic! s!"{type} is no valid type for padEmpty"

    | "zip" => match type with
                 | .fn (.expr (.array n s) a)
                       (.fn (.expr (.array _ t) _) (.expr (.array _ (.pair _ _)) _) ) =>
                     ...
                     mkLam (.fn xType yFunType) (mkName "x") yType
                           (mkLam yFunType (mkName "y") yType
                                  (mkZip n s t a (mkBvar 1 (mkName "x") xType) (mkBvar 0 (mkName "y") yType)))
                 | _ => panic! s!"{type} is no valid type for zip"

    | "unzip" => match type with
                   | .fn (.expr (.array n (.pair s t)) a)
                       ...
                       mkLam eFunType (mkName "e") eType
                             (mkUnzip n s t a (mkBvar 0 (mkName "e") eType))
                   | _ => panic! s!"{type} is no valid type for unzip"

    | "makePair" => match type with
                     | .fn (.expr s a) (.fn (.expr t _) (.expr (.pair _ _) _)) =>
                            ...
                            mkLam xFunType (mkName "x") xType
                                   (mkLam yFunType (mkName "y") yType
                                          (mkMakePair s t a (mkBvar 1 (mkName "x") xType) (mkBvar 0 (mkName "y") yType)))
                     | _ => panic! s!"{type} is no valid type for makePair"

    | "fst" => match type with
                 | .fn (.expr (.pair s t) .read) (.expr _ .read) =>
                     let eType := PhraseType.expr (.pair s t) .read
                     mkLam (.fn eType (.expr s .read)) (mkName "e") eType
                           (mkFst s t (mkBvar 0 (mkName "e") eType))
    ...
    | "asVectorAligned" => match type with
                     | .pi (.rise .nat) n
                           (.fn (.expr (.array mn _) a) (.expr (.array m (.vector _ t)) _)) =>
                            ...
                            mkDeplam      (.pi (.rise .nat) n eFunType) n (.rise .nat)
                                          (mkLam eFunType (mkName "e") eType
                                                 (mkAsVectorAligned (.bvar 1 n) m t a (mkBvar 0 (mkName "e") eType)))
                     | _ => panic! s!"{type} is no valid asVectorAligned type"

    | "asScalar" => match type with
                     | .fn (.expr (.array m (.vector n t)) a) (.expr (.array _ _) _) =>
                            let eType := PhraseType.expr (.array m (.vector n t)) a
                            let eFunType := PhraseType.fn eType (.expr (.array (.mult n m) t) a)
                            mkLam  eFunType (mkName "e") eType
                                   (mkAsScalar n m t a (mkBvar 0 (mkName "e") eType))
                     | _ => panic! s!"{type} is no valid asScalar type"

    | "map" => match type with
                | .fn (.fn (.expr s ai) (.expr t _)) (.fn (.expr (.array n _) _) (.expr (.array _ _) _)) =>
                    ...
                    mkLam fFunType (mkName "f") fType
                          (mkLam eFunType (mkName "e") eType
                                 (mkMap n s t ai (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
                | _ => panic! s!"{type} is no valid map type"

    | "mapSeq" => match type with
                    | .fn (.fn (.expr s .read) (.expr t .write)) (.fn (.expr (.array n _) .read) (.expr (.array _ _) .write)) =>
                        ...
                        mkLam fFunType (mkName "f") fType
                              (mkLam eFunType (mkName "e") eType
                                     (mkMapSeq false n s t (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
    ...
    | "mapSeqUnroll" => match type with
                          | .fn (.fn (.expr s .read) (.expr t .write)) (.fn (.expr (.array n _) .read) (.expr (.array _ _) .write)) =>
                              ...
                              mkLam fFunType (mkName "f") fType
                                    (mkLam eFunType (mkName "e") eType
                                           (mkMapSeq true n s t (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
                          | _ => panic! s!"{type} is no valid mapSeqUnroll type"

    | "mapStream" => match type with
                      | .fn (.fn (.expr s .read) (.expr t .write)) (.fn (.expr (.array n _) .read) (.expr (.array _ _) .read)) =>
                          let eType := (PhraseType.expr (.array n s) .read)
                          let fType := (PhraseType.fn (.expr s .read) (.expr t .write))
                          ...
                          mkLam fFunType (mkName "f") fType
                                (mkLam eFunType (mkName "e") eType
                                       (mkMapStream n s t (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
    ...
    | "iterateStream" => match type with
                          | .fn (.fn (.expr s .read) (.expr t .write)) (.fn (.expr (.array n _) .read) (.expr (.array _ _) .write)) =>
                              ...
                              mkLam fFunType (mkName "f") fType
                                    (mkLam eFunType (mkName "e") eType
                                           (mkIterateStream n s t (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
                          | _ => panic! s!"{type} is no valid p type"

    | "mapFst" => match type with
                     | .fn (.fn (.expr s a) (.expr s2 _)) (.fn (.expr (.pair _ t) _) (.expr (.pair _ _) _)) =>
                            ...
                            mkLam fFunType (mkName "f") fType
                                   (mkLam eFunType (mkName "e") eType
                                          (mkMapFst s t s2 a (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
                     | _ => panic! s!"{type} is no valid mapFst type"

    | "mapSnd" => match type with
                     | .fn (.fn (.expr t a) (.expr t2 _))
                           (.fn (.expr (.pair s _) _) (.expr (.pair _ _) _)) =>
                            ...
                            mkLam fFunType (mkName "f") fType
                                   (mkLam eFunType (mkName "e") eType
                                          (mkMapFst s t t2 a (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType)))
                     | _ => panic! s!"{type} is no valid mapSnd type"

    | "reduce" => panic! s!"reduce has no implementation"

    | "reduceSeq" => match type with
                      | .fn (.fn (.expr t .read) (.fn (.expr s .read) (.expr _ .write)))
                            (.fn (.expr _ .write) (.fn (.expr (.array n _) .read) (.expr _ .read))) =>
                                   ...
                                   mkLam fFunType (mkName "f") fType
                                          (mkLam iFunType (mkName "i") iType
                                                 (mkLam eFunType (mkName "e") eType
                                                        (mkReduceSeq false n s t (mkBvar 2 (mkName "f") fType) (mkBvar 1 (mkName "i") iType) (mkBvar 0 (mkName "e") eType))))
                              ...
                              | .fn (.fn (.expr t .read) (.fn (.expr s .read) (.expr _ .write)))
                                    (.fn (.expr _ .write) (.fn (.expr (.array n _) .read) (.expr _ .read))) =>
                                          ...
                                          mkLam fFunType (mkName "f") fType
                                                 (mkLam iFunType (mkName "i") iType
                                                        (mkLam eFunType (mkName "e") eType
                                                               (mkReduceSeq true n s t (mkBvar 2 (mkName "f") fType) (mkBvar 1 (mkName "i") iType) (mkBvar 0 (mkName "e") eType))))
                              | _ => panic! s!"{type} is no valid reduceSeq type"


    | "scanSeq" => match type with
                    | .fn (.fn (.expr s .read) (.fn (.expr t .read) (.expr _ .write)))
                          (.fn (.expr _ .write) (.fn (.expr (.array n _) .read) (.expr (.array _ _) .write))) =>
                            ...
                            mkLam fFunType (mkName "f") fType
                                   (mkLam iFunType (mkName "i") iType
                                          (mkLam eFunType (mkName "e") eType
                                                 (mkScanSeq n s t (mkBvar 2 (mkName "f") fType) (mkBvar 1 (mkName "i") iType) (mkBvar 0 (mkName "e") eType))))
                    | _ => panic! s!"{type} is no valid scanSeq type"

    | "iterate" => match type with
                     | .pi (.rise .nat) k
                           (.fn (.pi (.rise .nat) l (.fn (.expr (.array ln t) .read) (.expr (.array _ _) .write)))
                                (.fn   (.expr (.array insz _) .read) (.expr (.array m _) .write))) =>
                            let eType := PhraseType.expr (.array insz t) .read
                            ...
                            mkDeplam (.pi (.rise .nat) k (.fn fType eFunType)) k (.rise .nat)
                                   (mkLam (.fn fType eFunType) (mkName "f") fType
                                          (mkLam eFunType (mkName "e") eType
                                                 (mkIterate (.div ln (.bvar 0 l))  m (.bvar 2 k) t (mkBvar 1 (mkName "f") fType) (mkBvar 0 (mkName "e") eType))))
...
partial def expression (expr : RExprPt) : DPIAPhrase :=
  ...
  let pt := expr.type
  match node with
        | .bvar idx name => mkBvar idx name pt
        ...
        | .depapp fn arg => let eArg := DWrapper.rise arg
                            let eFn := expression fn
                            mkDepApp pt eFn eArg
        | .lam binderName _ body =>let ptBody := expression body
                                   match pt with
                                          | .fn binderType _ => mkLam pt binderName binderType ptBody
        ...
        | .deplam binderName binderKind body => let ptBinderKind := DKind.rise binderKind
                                                ...
                                                mkDeplam pt binderName ptBinderKind ptBody
