...
def getFreshAccessIdentifier (name : String := "tmp") (counter : Nat): Lean.Name :=
   mkName s!"{name}_{counter}"
        ...
        | e =>
            if accessIsRead E.type && notContainingArrayType (getDataType E.type)
                then match e with
                        ...
                        | _ => con E (mkLam (.fn E.type PhraseType.comm) (mkName "a") E.type
                                            (assignByType (getDataType E.type) A (mkBvar  0 (mkName "a") E.type))) counter
                        ...
                        | .ifThenElse cond thenP elseP => let ifThenElse :=  mkIfThenElse .comm (mkBvar 0 (mkName "x") cond.type) (acc thenP A counter) (acc elseP A counter)
                                                          let cont := mkLam (.fn cond.type .comm) (mkName "x") cond.type ifThenElse
                                                          con cond cont counter
...
partial def functionalAcc (func : FunctionalPrimitives) (type : PhraseType) (A: DPIAPhrase) (counter : Nat): DPIAPhrase :=
    match func with
                     ...
                     con val
                         (mkLam (.fn type .comm) (mkName "x") val.type
                                (assignByType (getDataType type) A
                                                                 (mkId (getDataType type) x (getAccess type)))) counter
                          ...
                          con lhs
                              (mkLam (.fn type .comm) (mkName "x") lhs.type
                                     (con rhs
                                          (mkLam (.fn type .comm) (mkName "y") rhs.type
                                                 (assignByType (getDataType type) A
                                                                                  (mkMod (getDataType type) x y))) counter)) counter
        ...
        | .makePair dt1 dt2 _ fst snd =>  mkSeq (acc fst (mkPairAcc1 dt1 dt2 A) counter)
                                                (acc snd  (mkPairAcc2 dt1 dt2 A) counter)
        | .map n dt1 dt2 _ f array => let x := mkBvar 0 (getFreshAccessIdentifier "fede_x" counter) (.expr dt1 .write)
                                      let oType := PhraseType.acc dt2
                                      let o := mkBvar 0 (getFreshAccessIdentifier "fede_o" counter) oType
                                      let i := mkBvar 0 (mkName "i") oType
                                      acc array
                                          (mkMapAcc n dt2 dt1
                                                    (mkLam  (.fn oType oType) (getFreshAccessIdentifier "fede_o" counter) oType
                                                            (fedAcc ((x, o) :: []) (applyCon f x)
                                                                    (mkLam (.fn oType oType) (mkName "i") oType i)
                                                                    (counter +1)))
                                                    A) (counter +1)
        | .mapFst dt1 dt2 dt3 _ f record => let x := mkBvar 0 (getFreshAccessIdentifier "fede_x" counter) (.expr dt1 .write)
                                                ...
                                                (mkMapFstAcc dt1 dt2 dt3
                                                            (mkLam  (.fn oType oType) (getFreshAccessIdentifier "fede_o" counter) oType
                                                                    (fedAcc ((x, o) :: []) (applyCon f x)
                                                                            ...
                                                                            (counter +1)))
                                                            A) (counter +1)
        | .mapSeq unroll n dt _ f array =>  let xType := PhraseType.expr (.array n dt) .read
                                            ...
                                            con  array
                                                 (mkLam (.fn xType .comm) (mkName "x") xType
                                                        (mkSeq  (mkComment "mapSeq")
                                                                (mkForLoop unroll n (mkLam (.fn iType .comm) (mkName "i") iType
                                                                                           (acc (applyCon f (atIndex x i))
                                                                                                (atIndex A i) counter)))))
                                                 counter
        | .mapSnd dt1 dt2 dt3 _ f record => let x := mkBvar 0 (getFreshAccessIdentifier "fede_x" counter) (.expr dt2 .write)
                                            let oType := PhraseType.acc dt3
                                            let o := mkBvar 0 (getFreshAccessIdentifier "fede_o" counter) oType
                                            let i := mkBvar 0 (mkName "i") oType
                                            acc record
                                                (mkMapFstAcc dt1 dt2 dt3
                                                            (mkLam  (.fn oType oType) (getFreshAccessIdentifier "fede_o" counter) oType
                                                                    (fedAcc ((x, o) :: []) (applyCon f x)
                                                                            ...
                                                                            (counter +1)))
                                                            A) (counter +1)
        | .mapVec n dt1 dt2 f array =>  let xType := PhraseType.expr (.vector n dt1) .read
                                        let x := mkBvar 2 (mkName "x") xType
                                        let iType := PhraseType.expr (.index n) .read
                                        ...
                                        con array
                                           (mkLam (.fn xType .comm) (mkName "x") xType
                                                  (mkForVec n dt2 A (mkLam  (.fn iType .comm) (mkName "i") iType
        ...
        | .scanSeq n dt1 dt2 f init array => let xType := PhraseType.expr (.array n dt1) .read
                                             ...
                                             let accum1 := mkBvar 1 (mkName "accum") accumType
                                             let iType := PhraseType.expr (.index n) .read
                                             let i := mkBvar 0 (mkName "i") iType
                                             con array
                                                (mkLam  (.fn xType .comm) (mkName "x") xType
                                                        (con init
                                                             (mkLam (.fn yType .comm) (mkName "y") yType
                                                                    (mkSeq  (mkComment "scanSeq")
                                                                            (mkNew  dt2
                                                                                    (mkLam  (.fn accumType .comm) (mkName "accum") accumType
                                                                                            (mkSeq  (acc y (mkProj2 (.acc dt2) accum0) counter)
                                                                                                    (mkForLoop  false n
                                                                                                                (mkSeq  (acc (applyCon  (applyCon f (atIndex x i))
                                                                                                                                        (mkProj1 (.expr dt2 .read) accum1))
                                                                                                                             (mkProj2 (.acc dt2) accum1) counter)
                                                                                                                        (assignByType dt2 (atIndex A i) (mkProj1 (.expr dt2 .read) accum1)))))))))
                                                             counter))
                                                counter
...
partial def con (E C : DPIAPhrase) (counter : Nat): DPIAPhrase :=
    match E.node with
...
partial def functionalCon (func : FunctionalPrimitives) (type : PhraseType) (C: DPIAPhrase) (counter : Nat): DPIAPhrase :=
    match func with
        | .id val => let x := mkBvar 0 (mkName "x") type
                     con val
                         (mkLam (.fn type .comm) (mkName "x") type
                                (applyCon C (mkId (getDataType type) x (getAccess type)))) counter
        | .neg val => let x := mkBvar 0 (mkName "x") type
                      con val
                          (mkLam (.fn type .comm) (mkName "x") type
                                 (applyCon C (mkNeg (getDataType type) x))) counter
        ...
        | .mod lhs rhs => let x := mkBvar 1 (mkName "x") type
                          ...
                          con lhs
                              (mkLam (.fn type .comm) (mkName "x") type
                                     (con rhs
                                          (mkLam (.fn type .comm) (mkName "y") type
                                                 (applyCon C (mkMod  (getDataType type) x y))) counter)) counter
        ...
        | .makePair dt1 dt2 a fst snd => let xType := PhraseType.expr dt1 .read
                                         ...
                                         con fst
                                             (mkLam (.fn xType  .comm) (mkName "x") xType
                                                    (con snd
                                                         (mkLam (.fn yType .comm) (mkName "y") yType
                                                                (applyCon C (mkMakePair dt1 dt2 a x y))) counter)) counter
        ...
        | .mapFst dt1 dt2 dt3 a f record => let x := mkBvar 0 (mkName "x") record.type
                                            con record
                                                (mkLam  (.fn record.type .comm) (mkName "x") record.type
                                                        (applyCon C (mkMapFst dt1 dt2 dt3 a f x))) counter
        ...
        | .mapSnd dt1 dt2 dt3 a f record => let x := mkBvar 0 (mkName "x") record.type
                                            con record
                                                (mkLam  (.fn record.type .comm) (mkName "x") record.type
                                                        (applyCon C (mkMapSnd dt1 dt2 dt3 a f x))) counter
        ...
        | .padCst n l r dt padExp array =>  let xType := PhraseType.expr (.array n dt) .read
                                            ...
                                            con array (mkLam (.fn xType .comm) (mkName "x") xType
                                                            (con padExp (mkLam  (.fn pType .comm) (mkName "p") pType
                                                                                (applyCon C (mkPadCst n l r dt p x ))) counter)) counter
        | .padClamp n l r dt array => let xType := PhraseType.expr (.array n dt) .read
                                      let x := mkBvar 0 (mkName "x") xType
                                      con array
                                          (mkLam (.fn xType .comm) (mkName "x") xType
                                                 (applyCon C (mkPadClamp n l r dt x))) counter
                                                          ...
                                                          (mkLam (.fn XType .comm) (mkName "X") XType
                                                                 (mkSeq (mkComment "reduceSeq")
                                                                        (mkNew dt2 (mkLam   (.fn (mkVar dt2) (.comm)) (mkName "accum") (mkVar dt2)
                                                                                            (mkSeq  (acc init (mkProj2 (.acc dt2) accum) counter)
                                                                                                    (mkSeq  (mkForLoop unroll n (mkLam  (.fn iType .comm) (mkName "i") iType
                                                                                                                                        (acc (applyCon  (applyCon f (mkProj1 (.expr dt2 .read) accum))
                                                                                                                                                        (atIndex X i))
                                                                                                                                             (mkProj2 (.acc dt2) accum) counter)))
                                                                                                            (applyCon C (mkProj1 (.expr dt2 .read) accum)))))))) counter
...
partial def functionalFed (env : Env) (func : FunctionalPrimitives) (C : DPIAPhrase) (counter : Nat): DPIAPhrase :=
    match func with
        ...
        | .map n dt1 dt2 a f array => let x := mkBvar 0 (getFreshAccessIdentifier "fede_x" counter) (.expr dt1 a)
                                      let oType := PhraseType.acc dt2
                                      let o := mkBvar 0 (getFreshAccessIdentifier "fede_o" counter) oType -- there is a probelm with the debruijn identifier for o that needs to be solved
                                      let y := mkBvar 0 (mkName "y") (getType env)
                                      fedAcc env array
                                             (mkLam (.fn (getType env) (.acc (.array n dt1))) (mkName "y") (getType env)
                                                    (mkMapAcc n
                                                              dt2
                                                              dt1
                                                              (mkLam (.fn oType oType) (getFreshAccessIdentifier "fede_o" counter) oType
                                                                     (fedAcc ((x,o) :: env)
                                                                             (applyCon f x)
                                                                             (mkLam (.fn oType oType) (mkName "i") oType
                                                                                    (mkBvar 0 (mkName "i") oType)) (counter +1)))
                                                              (applyCon C y)))
                                             (counter +1)
        | .mapFst dt1 dt2 dt3 _ f record => let x := mkBvar 0 (getFreshAccessIdentifier "fede_x" counter) (.expr dt2 .write)
                                            let oType := PhraseType.acc dt3
                                            let o := mkBvar 0 (getFreshAccessIdentifier "fede_o" counter) oType -- there is a probelm with the debruijn identifier for o that needs to be solved
                                            let y := mkBvar 0 (mkName "y") (getType env 2)
                                            fedAcc env record
                                                    (mkLam (.fn (getType env 2) (.acc (.pair dt1 dt2))) (mkName "y") (getType env 2)
                                                            (mkMapFstAcc dt1 dt2 dt3
                                                                        (mkLam (.fn oType oType) (getFreshAccessIdentifier "fede_o" counter) oType
                                                                                (fedAcc ((x,o) :: env)
                                                                                        (applyCon f x)
...
partial def functionalStr (func : FunctionalPrimitives) (type : PhraseType) (C : DPIAPhrase) (counter : Nat): DPIAPhrase :=
    match func with
